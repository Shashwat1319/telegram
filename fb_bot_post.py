...
def post_photo(product, msg):
    ...
    try:
        ...
        if not r.ok:
            log.warning("FB photo post failed: %s", r.text)
            return None
        j = r.json()
        return j if 'error' not in j else None
    except Exception as e:
        log.warning("FB photo post exception: %s", e)
        return None
...
def post_feed(product, msg):
    ...
    try:
        ...
        if not r.ok:
            log.warning("FB feed post failed: %s", r.text)
            return None
        j = r.json()
        return j if 'error' not in j else None
    except Exception as e:
        log.warning("FB feed post exception: %s", e)
        return None
