...
def load_products():
    try:
        ...
        log.error("Failed to load products: %s", e)
        return []
...
async def get_short_url(target_url):
    ...
    async with aiohttp.ClientSession() as s:
        try:
            ...
            log.warning("Short URL API failed: %s", e)
    ...
    try:
        async with Bot(token=BOT_TOKEN) as bot:
            ...
            for k in list(posted.keys()):
                if isinstance(posted[k], str):
                    posted[k] = {"last": posted[k], "count": 1}
            ...
            for product in to_post:
                ...
                if not await is_url_accessible(raw_link):
                    log.warning("Link not accessible: %s", raw_link)
                    continue
                ...
                try:
                    ...
                    log.error("Telegram error posting %s: %s", name, e)
                except Exception as e:
                    log.error("Failed to post %s: %s", name, e)

            await bot.shutdown()
    except Exception as e:
        log.error("post_deals fatal error: %s", e)
