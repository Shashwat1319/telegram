...
if os.path.exists(content_file):
    ...
    items = data['items']
    formats = sorted(set(i.get('format', '?') for i in items))
    print(f"\n[2/4] Content items: {len(items)} ({len(formats)} formats: {', '.join(formats)})")
...
print(f"\n[4/4] Bot commands: /start /deal /topdeal /search <kw> /referral /premium /about /contact")
print(f"\n{'=' * 60}")
print(f"  LIVE EXAMPLE: @smartgahr (Home & Kitchen Deals under ₹999)")
print(f"  Extra tools: promo_daily.py (daily promo posts), website_gen.py (SEO pages), validate_links.py (QA)")
