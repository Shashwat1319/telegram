...
def get_latest_product():
    ...
    try:
        ...
        log.error("Failed to load product: %s", e)
    return None
...
def get_font(size, bold=False):
    ...
    font_names = [
        "Oswald-Bold.ttf" if bold else "Oswald-Regular.ttf",
        "arial.ttf",
...
def generate_video(frame, output="shorts_deal.mp4", duration=5, fps=30):
    try:
        ...
        log.error("OpenCV (cv2) not installed. Run: pip install opencv-python")
        return False
